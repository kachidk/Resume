import React from "react";

const Resume = () => {
  return (
    <div class="resume">
      <div class="container">
        <section>
          <div>
            <div>
              <img
                src="https://media-exp1.licdn.com/dms/image/C4E03AQEFhgA--o0xRg/profile-displayphoto-shrink_200_200/0?e=1612396800&v=beta&t=y4N_fwVcDDz-608UjJXHaM-lPCX21gkVQQ71w7thf1Y"
                alt="avatar"
                class="avatar"
              />
            </div>
          </div>
          <div>
            <div class="my-name">Victor Nwanguma</div>
            <div class="my-title">Front-end &amp; Back-end Web Developer</div>
            <div class="links">
              <div class="link-item">
                <svg
                  viewBox="0 0 1792 1792"
                  xmlns="http://www.w3.org/2000/svg"
                  class="icon"
                >
                  <path d="M1520 1216q0-40-28-68l-208-208q-28-28-68-28-42 0-72 32 3 3 19 18.5t21.5 21.5 15 19 13 25.5 3.5 27.5q0 40-28 68t-68 28q-15 0-27.5-3.5t-25.5-13-19-15-21.5-21.5-18.5-19q-33 31-33 73 0 40 28 68l206 207q27 27 68 27 40 0 68-26l147-146q28-28 28-67zm-703-705q0-40-28-68l-206-207q-28-28-68-28-39 0-68 27l-147 146q-28 28-28 67 0 40 28 68l208 208q27 27 68 27 42 0 72-31-3-3-19-18.5t-21.5-21.5-15-19-13-25.5-3.5-27.5q0-40 28-68t68-28q15 0 27.5 3.5t25.5 13 19 15 21.5 21.5 18.5 19q33-31 33-73zm895 705q0 120-85 203l-147 146q-83 83-203 83-121 0-204-85l-206-207q-83-83-83-203 0-123 88-209l-88-88q-86 88-208 88-120 0-204-84l-208-208q-84-84-84-204t85-203l147-146q83-83 203-83 121 0 204 85l206 207q83 83 83 203 0 123-88 209l88 88q86-88 208-88 120 0 204 84l208 208q84 84 84 204z"></path>
                </svg>
                <a href="#">nwanguma.com</a>
              </div>
              <div class="link-item">
                <svg
                  viewBox="0 0 1792 1792"
                  xmlns="http://www.w3.org/2000/svg"
                  class="icon"
                >
                  <path d="M1664 1504v-768q-32 36-69 66-268 206-426 338-51 43-83 67t-86.5 48.5-102.5 24.5h-2q-48 0-102.5-24.5t-86.5-48.5-83-67q-158-132-426-338-37-30-69-66v768q0 13 9.5 22.5t22.5 9.5h1472q13 0 22.5-9.5t9.5-22.5zm0-1051v-24.5l-.5-13-3-12.5-5.5-9-9-7.5-14-2.5h-1472q-13 0-22.5 9.5t-9.5 22.5q0 168 147 284 193 152 401 317 6 5 35 29.5t46 37.5 44.5 31.5 50.5 27.5 43 9h2q20 0 43-9t50.5-27.5 44.5-31.5 46-37.5 35-29.5q208-165 401-317 54-43 100.5-115.5t46.5-131.5zm128-37v1088q0 66-47 113t-113 47h-1472q-66 0-113-47t-47-113v-1088q0-66 47-113t113-47h1472q66 0 113 47t47 113z"></path>
                </svg>
                <a href="mailto:nwangumav@gmail.com">nwangumav@gmail.com</a>
              </div>
            </div>
            <p>
              My primary interest is in web development, both front end and back
              end. The industry is fast paced and constantly moving forward. It
              is one that requires constant education throughout your entire
              career. This is exciting to me as there will always be new things
              to learn and new challenges to overcome. I primarily specialize on
              Laravel and React, with emphasis on the Laravel PHP framework
              and the ecosystem that surrounds it.
            </p>
          </div>
        </section>
        <section>
          <div class="section-title">Experience</div>
          <div>
            <div class="job">
              <div class="job-title-container">
                <div>
                  <div class="job-company">nwanguma.com</div>
                  <div class="job-title">
                    Freelance Web Developer / Designer
                  </div>
                </div>
                <div>Jan 2018 - Present</div>
              </div>
              <p>
                I design and develop websites and web applications to meet the
                business requirements of my clients. Every client is different
                so my work ranges from basic static websites to more complex web
                applications. Have a look at my online portfolio/website for
                examples of client work, code samples.
              </p>
            </div>
            <div class="job">
              <div class="job-title-container">
                <div>
                  <div class="job-company">XOWN Solutions</div>
                  <div class="job-title">Student Intern | Front End Developer</div>
                </div>
                <div>Oct 2020 - Nov 2020</div>
              </div>
              <p>
                Worked on several projects ranging from designing websites for clients
                and worked on React.js Projects. Responsibilities included implementing new
                features, website design, co-ordinating with other
                developers and project supervisors, improving code quality,
                improving existing projects and delvering the needs of our
                clients.
              </p>
            </div>
            <div class="job">
              <div class="job-title-container">
                <div>
                  <div class="job-company">
                    BUA INT'L Limited | Headquaters Lagos Nigeria
                  </div>
                  <div class="job-title">
                    Student Intern
                  </div>
                </div>
                <div>Mar 2020 - Aug 2020</div>
              </div>
              <p>
                Responsible for maintaining and proving IT support for
                several departments and personnel. I gained experience and knowledge from my superiors in all aspects
                including project management and design techniques, and also good team collaboration and rapour in projects. Furthermore, I
                automated repetitive tasks to make things easier to accomplish
                (i.e. application for medical or vacation leave, booking of drivers in an organisation, etc.).
              </p>
            </div>
          </div>
        </section>
        <section>
          <div class="section-title">Education</div>
          <div>
            <div class="job">
              <div class="job-title-container">
                <div>
                  <div class="job-company">
                    Honours Bachelor of Science in Computer Science Covenant University
                  </div>
                  <div class="job-title">Specialist: Computer Science</div>
                </div>
                <div>2017 - 2021</div>
              </div>
            </div>
          </div>
        </section>
        <section>
          <div class="section-title">Skills</div>
          <div>
            <div class="skills-container">
              <ul>
                <li>Laravel</li>
                <li>PHP</li>
                <li>MySQL</li>
                <li>HTML/CSS</li>
              </ul>
              <ul>
                <li>Tailwind CSS</li>
                <li>React.js</li>
                <li>JavaScript</li>
                <li>Vue.js</li>
              </ul>
              <ul>
                <li>Git/Version Control</li>
                <li>Servers/Deployment</li>
              </ul>
            </div>
            <p>
              I also enjoy helping or providing assistance to others, send me an Email at <br />
              <a href="mailto:nwangumav@gmail.com" class="font-bold">
                nwangumav@gmail.com
              </a>
            </p>
          </div>
        </section>
        <section>
          <div class="section-title">Interests</div>
          <div class="interests-container">
            <div>
              <svg
                xmlns="http://www.w3.org/2000/svg"
                width="37.935"
                height="46.898"
              >
                <path
                  fill-rule="evenodd"
                  clip-rule="evenodd"
                  fill="#404041"
                  d="M35.698 1.248c1.1.756 1.863 1.549 2.236 2.381-.483.807-1.121 1.707-1.914 2.68a11.816 11.816 0 0 1-2.636 2.397l-.612.044-6.941 10.705c1.599 1.244 2.674 2.461 3.219 3.68.725 1.749.631 3.288-.27 4.597-.909 1.321-1.721 2.154-2.436 2.537-1.298.641-2.02 1.055-2.145 1.238-.559.813-.74 1.76-.578 2.852.333 2.318-.046 4.826-1.131 7.542-1.079 2.701-3.324 4.353-6.742 4.929-2.109.374-5.115-.782-9.035-3.443C2.788 40.65.632 38.258.262 36.176c-.709-3.414.018-6.109 2.169-8.071 2.163-1.978 4.367-3.228 6.653-3.746 1.073-.241 1.896-.745 2.455-1.559.125-.183.254-1.006.402-2.437.062-.832.553-1.887 1.471-3.2.914-1.3 2.33-1.93 4.248-1.862 1.257.024 2.766.581 4.551 1.667l7.547-10.289-.216-.613a11.739 11.739 0 0 1 1.29-3.325c.61-1.101 1.221-2.016 1.828-2.74.91.053 1.925.484 3.038 1.247zM20.766 22.721c-1.915-1.317-3.477-1.037-4.702.852-1.316 1.913-1.035 3.518.855 4.817 1.877 1.293 3.474.979 4.789-.935 1.226-1.889.908-3.461-.942-4.734zM7.726 34.247l-.836 1.26 6.879 4.733.859-1.247-6.902-4.746zM31.107 5.036c.141.062.262.017.378-.153.114-.167.089-.313-.086-.431-.184-.129-.342-.108-.455.056-.117.171-.104.307.014.427l-.816 2.06L8.9 36.319l.149.102 21.229-29.33.829-2.055zm1.927-2.96l-2.213 5.387-20.837 29.6.188.132L30.97 7.565l2.214-5.388c.141.061.269.008.377-.15.117-.169.087-.313-.085-.431-.185-.125-.342-.114-.459.056-.109.158-.099.306.017.424zm2.681 1.842c.14.061.269.008.38-.15.115-.17.084-.313-.089-.431-.185-.129-.341-.114-.456.056-.11.158-.102.307.018.424l-4.016 4.15L11.11 37.839l.146.102 20.44-29.875 4.019-4.148zM33.9 6.958c.177.085.321.057.439-.113.114-.167.073-.321-.11-.447-.175-.121-.319-.091-.434.072-.099.147-.102.255-.018.348l-1.534 1.624-20.046 30.145.134.092L32.377 8.533 33.9 6.958z"
                ></path>
              </svg>
            </div>
            <div>
              <svg
                xmlns="http://www.w3.org/2000/svg"
                width="49.893"
                height="35.908"
              >
                <path
                  fill="#404041"
                  d="M0 0v35.908h49.893V0H0zm1.924 33.871V2.037h4.994v18.756h2.167v13.078H1.924zm7.939 0V20.792h2.165V2.037h5.307v18.756h2.166v13.078H9.863zm10.417 0V20.792h2.166V2.037h5.111v18.756h2.166v13.078H20.28zm10.223-13.079h2.166V2.037h5.111v18.756h2.166v13.078h-9.443V20.792zm17.466 13.079h-7.243V20.792h2.166V2.037h5.077v31.834z"
                ></path>
              </svg>
            </div>
            <div>
              <svg
                xmlns="http://www.w3.org/2000/svg"
                width="48.793"
                height="30.871"
              >
                <g fill="#404041">
                  <circle cx="11.911" cy="12.579" r="4.31"></circle>
                  <path d="M47.734 12.965C45.314 4.552 39.511-1.163 34.771.202c-1.316.377-2.402 1.277-3.246 2.541H17.268C16.423 1.48 15.336.58 14.019.202 9.281-1.163 3.478 4.552 1.058 12.965c-2.422 8.416-.542 16.34 4.197 17.706 3.983 1.146 8.714-2.724 11.559-8.982H31.98c2.847 6.259 7.575 10.128 11.561 8.982 4.736-1.366 6.617-9.291 4.193-17.706zm-35.823 6.057a6.44 6.44 0 0 1-6.443-6.443 6.441 6.441 0 0 1 6.443-6.441 6.442 6.442 0 1 1 0 12.884zm27.526-5.208H36.91v2.526h-3.076v-2.526h-2.526v-3.077h2.526V8.213h3.076v2.524h2.526v3.077z"></path>
                  <path d="M34.148 11.054h2.447V13.5h-2.447z"></path>
                </g>
              </svg>
            </div>
            <div>
              <svg
                xmlns="http://www.w3.org/2000/svg"
                width="49.3"
                height="46.911"
              >
                <g fill-rule="evenodd" clip-rule="evenodd" fill="#404041">
                  <path d="M44.272 33.393c.221-5.18-.505-9.533-2.86-10.408-.225-.087-1.021-.357-1.559-.393-3.771-.269-4.484 4.378-4.734 10.156-.25 5.775.06 10.496 3.831 10.762.536.037 1.35-.117 1.577-.168 2.411-.543 3.517-4.767 3.745-9.949z"></path>
                  <path d="M46.993 23.538c-.55-1.125-.995-1.786-1.376-2.203.121-.438.059-.906.059-.906a.821.821 0 0 0-.303-.472c-.398-1.533-1.262-5.031-3.625-8.836a25.78 25.78 0 0 0-.533-.82c.962-.264 1.55-1.009.958-2.144C37.977 3.176 31.619 0 24.501 0 17.51 0 11.254 3.072 7.057 7.908c-.5.918-.032 1.768.832 2.154-.247.355-.484.71-.699 1.059-2.361 3.805-3.225 7.303-3.623 8.835a.822.822 0 0 0-.303.473s-.068.528.086.987c-.44.373-.962 1.098-1.633 2.698C-.409 29.18-.271 36.27.581 39.323c.851 3.051 2.46 5.414 3.997 6.406 2.718 1.758 4.365 1.092 5.084.889.84-.24 1.003-1.197.834-2.518.284.016.594.02.842-.018 3.75-.539 3.796-5.271 3.228-11.018-.57-5.746-1.543-10.333-5.292-9.796-.533.075-1.311.406-1.531.507-2.304 1.043-2.788 5.441-2.277 10.6.512 5.152 1.854 9.287 4.289 9.648.02.004.066.01.096.012a7.867 7.867 0 0 1-1.212-.08c-2.437-.367-3.777-4.5-4.286-9.652-.467-4.691-.09-8.725 1.711-10.205-.38-1.426-.793-2.504-1.258-2.933a1.285 1.285 0 0 0-.008-.337c.234-1.117 1.18-5.06 3.609-8.955.365-.587.759-1.121 1.156-1.652a3.424 3.424 0 0 0 1.273-.563 23.12 23.12 0 0 1 13.665-4.432c5.208 0 10.008 1.701 13.851 4.568.328.222.691.377 1.062.479.382.515.766 1.032 1.12 1.601 2.281 3.658 3.254 7.362 3.558 8.725-.533.125-.958 1.146-1.308 2.639 2.308 1.047 2.789 5.442 2.28 10.598-.514 5.154-1.854 9.285-4.288 9.654a7.582 7.582 0 0 1-1.213.072c.018-.027.034-.059.081-.072-.05.004-.139.016-.241.025.013.025.013.023.023.045h-.03c-.2 1.625-.047 2.814.92 3.018.727.15 2.409.691 5.024-1.252 1.478-1.107 2.952-3.578 3.632-6.688.681-3.112.426-10.196-1.979-15.1z"></path>
                </g>
              </svg>
            </div>
            <div>
              <svg
                xmlns="http://www.w3.org/2000/svg"
                width="49.156"
                height="47.139"
              >
                <path
                  fill="#404041"
                  d="M48.47 0H.684A.81.81 0 0 0 0 .392v33.192c2.884.673 12.709 1.18 19.535 1.364v8.861h-5.911a1.665 1.665 0 1 0 0 3.33h22.127c.922 0 1.67-.745 1.67-1.668 0-.917-.748-1.662-1.67-1.662h-5.912v-8.833c6.542-.145 16.061-.581 19.316-1.18V.392A.81.81 0 0 0 48.47 0zM14.437 33.37a1.032 1.032 0 1 1 0-2.063 1.033 1.033 0 0 1 0 2.063zm3.042 0a1.03 1.03 0 1 1 0-2.063 1.032 1.032 0 1 1 0 2.063zm6.819 0c-2.286 0-4.14-.461-4.14-1.029 0-.571 1.854-1.034 4.14-1.034s4.139.463 4.139 1.034c0 .568-1.854 1.029-4.139 1.029zm7.08 0a1.032 1.032 0 1 1 0-2.063 1.033 1.033 0 1 1 0 2.063zm3.039 0a1.035 1.035 0 0 1-1.035-1.029 1.034 1.034 0 1 1 2.07 0 1.03 1.03 0 0 1-1.035 1.029zm10.905-3.579H3.833V3.176h41.489v26.615z"
                ></path>
              </svg>
            </div>
            <div>
              <svg
                xmlns="http://www.w3.org/2000/svg"
                width="48.75"
                height="36.649"
              >
                <path
                  fill="#404041"
                  d="M16.661 5.74l.902-1.008-.902-1.009.996.916 1-.916-.904 1.009.904 1.008-1-.912z"
                ></path>
                <ellipse
                  fill="#404041"
                  cx="23.952"
                  cy="18.324"
                  rx="3.12"
                  ry="3.152"
                ></ellipse>
                <path
                  fill="#404041"
                  d="M43.624 8.172L12.149.207c-3.63-.918-7.304 1.303-8.215 4.97L.205 20.184c-.908 3.663 1.294 7.375 4.923 8.293L36.6 36.441c3.629.922 7.307-1.304 8.219-4.969l3.727-15.008c.908-3.663-1.296-7.374-4.922-8.292zm-28.555.512L9.733 7.568c-.919-.194-1.574-.81-1.461-1.375l.674-3.273c.117-.567.958-.869 1.879-.675l5.334 1.112c.921.191 1.577.811 1.458 1.375l-.667 3.272c-.117.568-.959.871-1.881.68zm8.881 17.42c-4.256 0-7.705-3.484-7.705-7.779 0-4.297 3.449-7.779 7.705-7.779 4.257 0 7.708 3.482 7.708 7.779.001 4.294-3.45 7.779-7.708 7.779z"
                ></path>
              </svg>
            </div>
            <div>
              <svg
                xmlns="http://www.w3.org/2000/svg"
                width="49.595"
                height="33.031"
              >
                <g fill="#404041">
                  <path d="M29.137 25.976c0 1.482-1.24 2.684-2.771 2.684h-12.08c-1.53 0-2.771-1.201-2.771-2.684v-5.139c0-1.482 1.241-2.682 2.771-2.682h12.079c1.531 0 2.771 1.199 2.771 2.682v5.139z"></path>
                  <path d="M36.734 7.997H29.07L24.811.819V.678H16.44C16.18.268 15.79 0 15.346 0H6.913C6.13 0 5.496.815 5.496 1.821s.634 1.822 1.417 1.822h8.433c.444 0 .834-.267 1.095-.678h6.984l2.985 5.032H5.742C2.572 7.997 0 10.352 0 13.254v14.521c0 2.902 2.572 5.256 5.742 5.256h30.993c4.145 0 4.387-5.445 5.285-7.271V14.75c-.795-1.914-.268-6.164-5.286-6.753zm-5.992 18.585c0 1.883-1.467 3.411-3.276 3.411H13.187c-1.809 0-3.276-1.528-3.276-3.411v-6.537c0-1.885 1.467-3.41 3.276-3.41h14.279c1.81 0 3.276 1.525 3.276 3.41v6.537zM49.518 11.363l-6.783 3.452c-.023-.05-.023 10.75 0 10.71l6.783 3.096c.091-7.408.111-14.643 0-17.258z"></path>
                </g>
              </svg>
            </div>
            <div>
              <svg
                xmlns="http://www.w3.org/2000/svg"
                width="24.944"
                height="47.432"
              >
                <g fill-rule="evenodd" clip-rule="evenodd" fill="#404041">
                  <path d="M23.58 19.705c-.453.457-2.572 0-2.572 0v26.168c0 1.043-.84 1.559-2.529 1.559-1.807 0-2.705-.516-2.705-1.559V19.705s-1.965.427-2.391 0c-3.068-3.078 0-17.432 0-17.432L14.115 0l.537 2.273v10.77h3.131V2.273L18.494 0l.711 2.273v10.77h2.922V2.273L22.867 0l.713 2.273s3.07 14.354 0 17.432zM7.062 45.973c0 .977-.746 1.459-2.242 1.459-1.693 0-2.541-.482-2.541-1.459V27.788L.304 23.619S-2.182 5.822 7.062 0c-.268 7.881 0 45.973 0 45.973z"></path>
                </g>
              </svg>
            </div>
          </div>
        </section>
      </div>
    </div>
  );
};

export default Resume;
